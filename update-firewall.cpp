 ...
 * /usr/local/boost_1_78_0/stage/lib/ -lboost_program_options -lpthread -lcrypto
 * -std=c++20 xxx.cpp
 * */
...
int main(int argc, char* argv[])
{
    try
    {
        auto desc_help = "Usage: update-firewall\t[--verbose | -v] [--help | -h] "
                         ...
                         "Example:\n"
                         " source the env file which stores the <token_var_key>, "
                         "so it's available"
                         " in shell environment, and then run this executable.\n"
                         "    update-firewall -a -d -p ssh my-firewall\n"
                         "    update-firewall -a -d -p ssh -t "
                         "DIGITALOCEAN_ACCESS_TOKEN my-firewall\n"
    ...
    options.add_options()
        ...
        if (!vm.count(token_var_key) &&
            vm[token_var_key].as<std::string>().empty())
        {
       ...
       * {"firewalls": [
	   * 	{"id":"xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx","name":"my-firewall","status":"succeeded",
	   * 		"inbound_rules":[
	   * 			{"protocol":"tcp","ports":"22","sources":{"addresses":["100.100.10.100",]}},
	   ...
	   * 		],
	   * 		"created_at":"2021-01-00T00:00:00Z","droplet_ids":[111111111],"tags":[],
	   * 		"pending_changes":[]},
	   * 	],
	   * "links":{},"meta":{"total":1}
	   * }
            ...
            if (data_raw.is_object())
            {
                ...
                for (auto& f : firewalls)
                {
                   ...
                   * required addresses to avoid an unnecessary put request. */
                        ...
                        const std::string& payload = serialize(f);
